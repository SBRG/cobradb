define([], function() {
  ...
  function venn() {
    ...
    out.venn = function(sets, overlaps, parameters) {
        parameters = parameters || {};
        parameters.maxIterations = parameters.maxIterations || 500;
        var lossFunction = parameters.lossFunction || out.lossFunction;
        var initialLayout = parameters.layoutFunction || out.greedyLayout;
    ...
    out.distanceFromIntersectArea = function(r1, r2, overlap) {
        // handle complete overlapped circles
        if (Math.min(r1, r2) * Math.min(r1,r2) * Math.PI <= overlap) {
            return Math.abs(r1 - r2);
        }

        return out.bisect(function(distance) {
            //console.log(circleIntersection());
    ...
    out.getDistanceMatrix = function(sets, overlaps) {
        ...
        for (i = 0; i < overlaps.length; ++i) {
            ...
            var left = current.sets[0],
                ...
                distance = out.distanceFromIntersectArea(r1, r2, current.size);
    ...
    out.greedyLayout = function(sets, overlaps) {
        ...
        var distances = out.getDistanceMatrix(sets, overlaps);
        ...
        for (i = 1; i < mostOverlapped.length; ++i) {
            ...
            for (j = 0; j < points.length; ++j) {
                ...
                var loss = out.lossFunction(sets, overlaps);
                if (loss < bestLoss) {
                    bestLoss = loss;
                    bestPoint = points[j];
                }
    ...
    out.classicMDSLayout = function(sets, overlaps) {
    ...
    out.lossFunction = function(sets, overlaps) {
        var output = 0;
    ...
    out.scaleSolution = function(solution, width, height, padding) {
     ...
     * have opposite signs */
    out.bisect = function(f, a, b, parameters) {
        parameters = parameters || {};
    ...
    out.fmin = function(f, x0, parameters) {
        parameters = parameters || {};
    ...
    out.drawD3Diagram = function(element, dataset, width, height, parameters) {
        parameters = parameters || {};
        ...
        dataset = out.scaleSolution(dataset, width, height, padding);
    ...
    out.updateD3Diagram = function(element, dataset) {
        var svg = element.select("svg"),
            width = parseInt(svg.attr('width'), 10),
            height = parseInt(svg.attr('height'), 10);

        dataset = out.scaleSolution(dataset, width, height, 6);
  ...
  function circleIntersection() {
    ...
    out.intersectionArea = function(circles, stats) {
        // get all the intersection points of the circles
        var intersectionPoints = getIntersectionPoints(circles);

        // filter out points that aren't included in all the circles
        var innerPoints = intersectionPoints.filter(function (p) {
            return out.containedInCircles(p, circles);
        });
        ...
        if (innerPoints.length > 1) {
            ...
            var center = out.getCenter(innerPoints);
            ...
            for (i = 0; i < innerPoints.length; ++i) {
                ...
                arcs.push(arc);
                arcArea += out.circleArea(arc.circle.radius, arc.width);
                p2 = p1;
            }
        } else {
            ...
            for (i = 0; i < circles.length; ++i) {
                if (out.distance(circles[i], smallest) > Math.abs(smallest.radius - circles[i].radius)) {
                    disjoint = true;
                    break;
                }
            }
    ...
    out.containedInCircles = function(point, circles) {
        for (var i = 0; i < circles.length; ++i) {
            if (out.distance(point, circles[i]) > circles[i].radius + SMALL) {
                return false;
            }
        }
        return true;
    ...
    function getIntersectionPoints(circles) {
        ...
        for (var i = 0; i < circles.length; ++i) {
            for (var j = i + 1; j < circles.length; ++j) {
                var intersect = out.circleCircleIntersection(circles[i],
                                                              circles[j]);
    ...
    out.circleIntegral = function(r, x) {
        var y = Math.sqrt(r * r - x * x);
        return x * y + r * r * Math.atan2(x, y);
    };

    /** Returns the area of a circle of radius r - up to width */
    out.circleArea = function(r, width) {
        return out.circleIntegral(r, width - r) - out.circleIntegral(r, -r);
    };


    /** euclidean distance between two points */
    out.distance = function(p1, p2) {
    ...
    out.circleOverlap = function(r1, r2, d) {
        // no overlap
        if (d >= r1 + r2) {
            return 0;
        }
        ...
        return out.circleArea(r1, w1) + out.circleArea(r2, w2);
    };
    ...
    out.circleCircleIntersection = function(p1, p2) {
        var d = out.distance(p1, p2),
            r1 = p1.radius,
            r2 = p2.radius;
